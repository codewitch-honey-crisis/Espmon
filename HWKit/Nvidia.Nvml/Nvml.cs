...
namespace Nvidia.Nvml
{
    ...
    public struct NvmlVgpuInstanceUtilization
    {
        public ulong Timestamp;          //!< CPU Timestamp in microseconds
        public uint SMUtil;             //!< SM (3D/Compute) Util Value
        public uint MemUtil;            //!< Frame Buffer Memory Util Value
        public uint EncUtil;            //!< Encoder Util Value
        public uint DecUtil;            //!< Decoder Util Value
        public uint JpgUtil;            //!< Jpeg Util Value
        public uint OfaUtil;            //!< Ofa Util Value
    ...
    {
        public uint fan;
        public uint speed; // OUT: the fan speed in RPM.
        public uint version; // the API version
    }
    internal static class Api
    {
        ...
        [DllImport(NVML_SHARED_LIBRARY_STRING, CharSet = CharSet.Ansi, EntryPoint = "nvmlDeviceGetFanSpeedRPM")]
        internal static extern NvmlReturn NvmlDeviceGetFanSpeedRpm(IntPtr device, ref NvmlFanSpeedInfo info);
    ...
    public class NvGpu
    {
        ...
        public static string NvmlDeviceGetName(IntPtr device, uint length = 64)
        {
        ...
        public static uint NvmlDeviceGetClock(IntPtr device, NvmlClockType type, NvmlClockId id)
        {
            uint result = 0;
            NvmlReturn res = Api.NvmlDeviceGetClock(device, type, id, out result);
            if (NvmlReturn.NVML_SUCCESS != res)
            {
                throw new SystemException(res.ToString());
            }
        ...
        public static uint NvmlDeviceGetFanSpeed(IntPtr device,NvmlDeviceArchitecture fan)
        {
            NvmlReturn res;
            uint speed;
        ...
        public static NvmlAccountingStats NvmlDeviceGetUtilizationRates(IntPtr device)
        {
            ...
            var res = Api.NvmlDeviceGetUtilizationRates(device, out util);
            if(res != NvmlReturn.NVML_SUCCESS)
            {
                throw new SystemException(res.ToString());
            }
            return util;
