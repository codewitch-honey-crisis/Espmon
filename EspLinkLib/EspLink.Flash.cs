...
namespace EL
{
	partial class EspLink
    {
		...
		async Task<uint> FlashBeginAsync(uint size, uint compsize, uint offset, uint blockSize, int timeout = -1, CancellationToken cancellationToken = default)
        {
			CheckReady();
			if (Device == null) throw new InvalidOperationException("No device connected");
		...
		async Task FlashFinishAsync(bool reboot = false, int timeout = -1, CancellationToken cancellationToken = default)
        {
			...
			await CheckCommandAsync("leave compressed flash mode", Device.ESP_FLASH_END, data, 0, timeout, cancellationToken);
			_inBootloader = false;
		}
		async Task FlashBlockAsync(byte[] data, uint seq, int attempts = 3, int timeout = -1, CancellationToken cancellationToken = default)
        {
			...
			while (attempts-- > 0)
			{
				try
				{
                    ...
                    Device.ESP_FLASH_DATA, pck, Checksum(data, 0, data.Length), timeout, cancellationToken);
					return;
		...
		async Task<uint> FlashDeflBeginAsync(uint size, uint compsize, uint offset, uint blockSize, int timeout = -1, CancellationToken cancellationToken = default)
        {
            CheckReady();
            if (Device == null) throw new InvalidOperationException("No device connected");
            // Start downloading compressed data to Flash (performs an erase)
		...
		async Task FlashDeflFinishAsync(bool reboot = false, int timeout = -1, CancellationToken cancellationToken = default)
        {
            // not sure this even should be used
			CheckReady();
            if (Device == null) throw new InvalidOperationException("No device connected");
            ...
            var data = BitConverter.GetBytes(not_reboot);
            await CheckCommandAsync("leave compressed flash mode", Device.ESP_FLASH_DEFL_END, data, 0, timeout, cancellationToken);
            _inBootloader = false;
		}
        async Task FlashDeflBlockAsync(byte[] data, uint seq, int attempts = 3, int timeout = -1, CancellationToken cancellationToken = default)
        {
			CheckReady();
            if (Device == null) throw new InvalidOperationException("No device connected");
            if (attempts < 1) attempts = 1;
            ...
            while(attempts-->0)
            {
                try
                {
                    ...
                    await CheckCommandAsync(
                    $"write compressed data to flash after seq {seq}",
                    Device.ESP_FLASH_DEFL_DATA, pck, Checksum(data, 0, data.Length), timeout, cancellationToken);
        ...
        {
            FlashAsync(uncompressedInput, compress, blockSize, offset, writeAttempts, finalize, timeout, progress, CancellationToken.None).Wait();
        }
		/// <summary>
		...
		public async Task FlashAsync(Stream uncompressedInput, bool compress = true, uint blockSize = 0, uint offset = 0x10000, int writeAttempts = 3, bool finalize = false, int timeout = -1, IProgress<int>? progress = null, CancellationToken cancellationToken = default)
        {
            CheckReady();
            if (Device == null) throw new InvalidOperationException("No device connected");
            if (blockSize==0)
			...
			if (compress)
			{
				blockCount = await FlashDeflBeginAsync(uclen, cln, offset, blockSize, timeout, cancellationToken);
			} else
			{
				blockCount = await FlashBeginAsync(uclen, cln, offset, blockSize, timeout, cancellationToken);
			}
            ...
            for(int i = 0;i<blockCount;++i)
            {
				...
				if (compress)
				{
					await FlashDeflBlockAsync(block, (uint)i, writeAttempts, timeout, cancellationToken);
				} else
				{
					await FlashBlockAsync(block, (uint)i, writeAttempts, timeout, cancellationToken);
				}
            ...
            if (IsStub) {
                ...
                await ReadRegAsync(Device.CHIP_DETECT_MAGIC_REG_ADDR, timeout, cancellationToken);
            }
			if (finalize)
            {
				...
				if (compress)
				{
					await FlashDeflFinishAsync(false, timeout, cancellationToken);
				} else
				{
					await FlashFinishAsync(false, timeout, cancellationToken);
				}
            }
			progress?.Report(100);

		}
		async static Task CompressToZlibStreamAsync(Stream inputStream, Stream compressedStream,CancellationToken cancellationToken=default)
		{
