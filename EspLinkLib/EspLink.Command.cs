...
namespace EL
{
	...
	{
		internal async Task<(uint Value, byte[] Data)> CommandAsync(int op = -1, byte[]? data = null, uint chk = 0, int timeout = -1, CancellationToken cancellationToken = default)
        {
			if (op != -1)
			{
		...
		internal async Task<uint> CommandResultAsync(int op = -1, byte[]? data = null, uint chk = 0, int timeout = -1, CancellationToken cancellationToken = default)
        {
			if (op != -1)
			{
				if (data == null) data = Array.Empty<byte>();
		...
		internal async Task<(uint Value, byte[] Data)> CheckCommandAsync(string op_description, int op, byte[] data, uint chk, int timeout = -1, CancellationToken cancellationToken = default)
        {
			(uint Value, byte[] Data) ret= await CommandAsync(op, data, chk, timeout, cancellationToken);
			
			if (Device==null || ret.Data.Length < Device.STATUS_BYTES_LENGTH)
			{
