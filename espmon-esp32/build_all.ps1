...
$commonPath = Join-Path $commonPath "common"

$buffersPath = Join-Path $PSScriptRoot "htcw_buffers"

$buffersGenPath = Join-Path $buffersPath "buffers_gen_c.py"
$interfacePath = Join-Path $commonPath "interface.h"
$buffersTargetPath = Join-Path $PSScriptRoot "shared"
$buffersTargetPath = Join-Path $buffersTargetPath "main"

#./nanopb_generator/nanopb_generator.py -I ../common espmon.proto -D ../common
Write-Host "Generating htcw_buffers code" -ForegroundColor Gray
& python.exe $buffersGenPath --buffers --out $buffersTargetPath $interfacePath
...
$boardsJsonTime = (Get-Item $boardsJsonPath).LastWriteTime
...
foreach ($board in $boards) {
    ...
    if (-not (Test-Path $ymlPath) -or (Get-Item $ymlPath).LastWriteTime -lt $boardsJsonTime) {
        ...
        Write-Host "  Skipped: idf_component.yml (up to date)" -ForegroundColor DarkGray
    }
    ...
    if (-not (Test-Path $boardConfigPath) -or (Get-Item $boardConfigPath).LastWriteTime -lt $boardsJsonTime) {
        ...
        Write-Host "  Skipped: board_config.h (up to date)" -ForegroundColor DarkGray
    }

    # Write CMakeLists.txt for this board
    $cmakePath = Join-Path $PSScriptRoot "boards"
    $cmakePath = Join-Path $cmakePath $boardSlug
    $cmakePath = Join-Path $cmakePath "CMakeLists.txt"
    if (-not (Test-Path $cmakePath) -or (Get-Item $cmakePath).LastWriteTime -lt $boardsJsonTime) {
        ...
        foreach ($define in $board.defines) {
        ...
        Set-Content -Path $cmakePath -Value $cmakeLines -Encoding UTF8
        Write-Host "  Written: CMakeLists.txt" -ForegroundColor Gray
        $needsClean = $true
    } else {
        Write-Host "  Skipped: CMakeLists.txt (up to date)" -ForegroundColor DarkGray
    }

    # If any generated files changed, wipe build artifacts so CMake/IDF starts fresh
    if ($needsClean) {
        $boardDir = Join-Path $PSScriptRoot "boards" | Join-Path -ChildPath $boardSlug
        $buildDir = Join-Path $boardDir "build"
        $managedComponentsDir = Join-Path $boardDir "managed_components"
        foreach ($cleanDir in @($buildDir, $managedComponentsDir)) {
            if (Test-Path $cleanDir) {
                Remove-Item $cleanDir -Recurse -Force
                Write-Host "  Cleaned: $($cleanDir | Split-Path -Leaf)" -ForegroundColor DarkYellow
            }
        }
    }
