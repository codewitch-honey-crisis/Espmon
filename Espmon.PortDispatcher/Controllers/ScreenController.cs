...
public sealed class ScreenController : ControllerBase
{
    ...
    public ScreenController(PortController parent, string name) : base(parent)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(name,nameof(name));
        Parent = parent;
        _name = name;
        _top = new ScreenValuesController(this);
        _bottom = new ScreenValuesController(this);
    }
    string _name;
    public string Name
    {
        ...
        set
        {
            ...
            if (_name.Equals(value, StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    throw new ArgumentException("A screen with that name already exists");
                }
    ...
    public ScreenInterval Interval
    {
        ...
        set
        {
            if (_interval != value)
            {
                UpdateProperty(nameof(Interval), () => _interval = value);
            }
        }
    }
    private ScreenValuesController _top;
    public ScreenValuesController Top
    {
        get { return _top; }
        set
        {
            if (_top != value)
            {
                UpdateProperty(nameof(Top), () => _top = value);
            }
        }
    }
    private ScreenValuesController _bottom;
    public ScreenValuesController Bottom
    {
        get { return _bottom; }
        set
        {
            if (_bottom != value)
            {
                UpdateProperty(nameof(Bottom),()=>_bottom=value);
            }
        }
    }
    internal static ScreenController FromJson(PortController parent, string name, JsonObject json)
    {
        var result = new ScreenController(parent, name);
        ...
        if (json.TryGetValue("top", out var top))
        {
            ...
            {
                result.Top = ScreenValuesController.FromJson(result, obj);
            }
            else
            {
        ...
        if (json.TryGetValue("bottom", out var bottom))
        {
            ...
            {
                result.Bottom = ScreenValuesController.FromJson(result, obj);
            }
            else
            {
    ...
    private static string ToJsonColor(string col)
    {
        ...
        sb.Append(char.ToLowerInvariant(col[0]));
